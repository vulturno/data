...
for (( i=0; i<${#nombre[@]}; ++i )); do
    ...
    jq -r '["fecha", "maxima", "minima", "tmedia"], (.[] | select(.tmax != null) | select(.tmin != null) | select(.tmed != null) | [.fecha, .tmax, .tmin, .tmed ]) | @csv' ~/github/data/diarias/"${indicativo[$i]}"-total-diario.json > ~/github/data/temp-media/"${nombre[$i]}"-diarias.csv

    echo "${nombre[$i]} terminada!"
done

# jq deja algunas comillas en el CSV, las eliminamos con sed :)
sed -i 's/\"//g' ~/github/data/temp-media/*.csv
