...
sed -i 's/\"//g' ~/github/data/temp-45/*.csv

# Concatenamos todos los CSV de heladas en el mismo
cat ~/github/data/temp-45/*.csv > ~/github/data/temp-45/total-temp-45.csv &&
...
sed -i '2,${/fecha/d;}'  ~/github/data/temp-45/total-temp-45.csv

# Recorremos el array de años
for (( i=0; i<${#year[@]}; ++i )); do
    csvgrep -c fecha -r "(${year[$i]})" ~/github/data/temp-45/total-temp-45.csv | csvstat -c fecha --count --csv >> ~/github/data/temp-45/count-temp-45.csv
done

# Eliminamos los row count que produce csvkit
sed -i 's/Row count: //g' ~/github/data/temp-45/count-temp-45.csv &&
# Ahora vamos a crear un CSV con los años y el total de cada año
csvjoin -u 1 ~/github/data/year.csv ~/github/data/temp-45/count-temp-45.csv > ~/github/data/total-temp-45.csv

# Añadimos el header con year y total al csv
sed -i '1s/^/year,total\n/' ~/github/data/total-temp-45.csv
