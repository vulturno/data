...
for (( i=0; i<${#nombre[@]}; ++i )); do
    ...
    jq -r '["fecha", "minima"], (.[] | select(.tmin >= 25) | select(.tmin != null) | [.fecha, .tmin]) | @csv' ~/github/data/diarias/"${indicativo[$i]}"-total-diario.json > ~/github/data/torridas/"${nombre[$i]}"-torridas.csv
    echo "${nombre[$i]} terminada!"
done

# jq deja algunas comillas en el CSV, las eliminamos con sed :)
sed -i 's/\"//g' ~/github/data/torridas/*.csv

# Concatenamos todos los CSV de heladas en el mismo
cat ~/github/data/torridas/*.csv > ~/github/data/torridas/total-torridas.csv &&
...
sed -i '2,${/fecha/d;}'  ~/github/data/torridas/total-torridas.csv

# Recorremos el array de años
for (( i=0; i<${#year[@]}; ++i )); do
    csvgrep -c fecha -r "(${year[$i]})" ~/github/data/torridas/total-torridas.csv | csvstat -c fecha --count --csv >> ~/github/data/torridas/count-torridas.csv
done

# Eliminamos los row count que produce csvkit
sed -i 's/Row count: //g' ~/github/data/torridas/count-torridas.csv &&
# Ahora vamos a crear un CSV con los años y el total de cada año
csvjoin -u 1 -y 0 ~/github/data/year.csv ~/github/data/torridas/count-torridas.csv > ~/github/data/total-torridas.csv
