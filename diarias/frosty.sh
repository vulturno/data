...
Hay estaciones que alguna vez no han registrado estos valores
Así que no los seleccionamos: select(.tmin != null)
...
for (( i=0; i<${#nombre[@]}; ++i )); do
    ...
    jq -r '["fecha", "min"], (.[] | select(.tmin <= 0) | select(.tmin != null) | [.fecha, .tmin]) | @csv' ${indicativo[$i]}-total-diario.json > ~/github/data/heladas/${nombre[$i]}-heladas.csv
